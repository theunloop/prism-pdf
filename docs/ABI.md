...
| `prismpdf_image_source_from_jpeg` / `_from_png` / `_from_rgb` / `_from_gray` / `_from_rgba` | An image to **place** — distinct from `PrismPdfImage`, which is one *extracted*. `_from_rgba` turns alpha into an `/SMask` (§11.6.5.2); `_from_png` decodes a PNG file onto the same paths (alpha → `/SMask`, palette expanded, 16-bit → 8). Return null on a bad length, an unusable JPEG, or a PNG the decoder refuses. |
