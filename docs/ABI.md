...
| `prismpdf_caption_style_set_enabled` / `_font` / `_size` / `_leading` / `_wrap` / `_placement` | Set one aspect of the caption. `enabled` false draws no caption at all; `placement` `Below` gives it the widget's full width instead of the 60% left beside a graphic. |
