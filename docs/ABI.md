...
| `prismpdf_caption_style_new` / `_free` | Create/release reusable caption typography; a fresh style holds the historical Helvetica 8pt beside the graphic. |
| `prismpdf_caption_style_set_enabled` / `_font` / `_size` / `_leading` / `_wrap` / `_placement` | Set one aspect of the caption. `enabled` false draws no caption at all; `placement` `Below` gives it the widget's full width instead of the 60% left beside a graphic. `_size` takes a finite size above zero and `_leading` a finite one, rejecting anything else with `NullArgument`: a size that is not a number cannot be written to a content stream, and a signed revision carrying one cannot be corrected in place. |
| `prismpdf_sign_settings_set_appearance_image_styled(settings, page_index, rect, image, text, style)` | A visible widget whose caption uses that style. `image` may be null for a caption-only widget, and a null `style` is the historical default. |
...
| `prismpdf_image_source_from_jpeg` / `_from_png` / `_from_rgb` / `_from_gray` / `_from_rgba` | An image to **place** — distinct from `PrismPdfImage`, which is one *extracted*. `_from_rgba` turns alpha into an `/SMask` (§11.6.5.2); `_from_png` decodes a PNG file onto the same paths (alpha → `/SMask`, palette expanded, 16-bit → 8). Return null on a bad length, an unusable JPEG, or a PNG the decoder refuses — which includes one over the size bound: each side at most 1048576 pixels, multiplying to at most 268435456 (the buffer is sized from the header before any pixel data is read). |
