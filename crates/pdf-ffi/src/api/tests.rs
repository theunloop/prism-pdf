...
#[test]
fn png_image_sources_decode_or_return_null() {
    // A 2×2 RGBA PNG (stdlib-encoded): decodes to a 2×2 source; junk and a truncation do not.
    const RGBA_2X2: &[u8] = &[
    ...
    let image = unsafe { prismpdf_image_source_from_png(RGBA_2X2.as_ptr(), RGBA_2X2.len()) };
    assert!(!image.is_null());
    let (mut w, mut h) = (0u32, 0u32);
    assert_eq!(
        unsafe { prismpdf_image_source_size(image, &mut w, &mut h) },
        PrismPdfStatus::Ok
    );
    assert_eq!((w, h), (2, 2));
    unsafe { prismpdf_image_source_free(image) };
    assert!(unsafe { prismpdf_image_source_from_png(b"junk".as_ptr(), 4) }.is_null());
